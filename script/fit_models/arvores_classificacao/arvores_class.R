...
melhor_cp <- ajuste_arvore$cptable[which.min(ajuste_arvore$cptable[ , "xerror"]), "CP"]

# pruned tree

ajuste_poda <- rpart::prune(
  ajuste_arvore, 
  cp = melhor_cp
)

rpart.plot(ajuste_poda) 

# Performance model -------------------

# classification rule

class_arvore <- predict(
  ajuste_poda, 
  newdata = dados_teste, 
  type = "class"
)
...
medidas_arvore <- medidas_desempenho(
